[<AutoOpen>]
module Finelines.Tasks.AzureAppServiceManage.AzureAppServiceManageStartTask

open Finelines
open Finelines.Tasks

type AzureAppServiceManageStartRawTask =
    { Task: Task
      AzureAppServiceManageRawTask: AzureAppServiceManageRawTask 
      Slot: Parameter<DeploymentSlot> }

type AzureAppServiceManageStartTask =
    { Task: Task
    ...
    interface IYamlTask with
        member task.AsYamlTask = {
            ...
            Inputs = 
                [
                    "action", Text "Start Azure App Service"
                ] @ ([
...
type AzureAppServiceManageStartTaskBuilder() =
    member _.Yield _ : AzureAppServiceManageStartRawTask =
        { Task = Task.Default
          AzureAppServiceManageRawTask = AzureAppServiceManageRawTask.Default 
          Slot = Parameter.Unset None }

    member _.Run (task: AzureAppServiceManageStartRawTask) =
        { Task = task.Task
    ...
    member _.AddSubscription(task: AzureAppServiceManageStartRawTask, subscription) =
    ...
    member _.AddAppName(task: AzureAppServiceManageStartRawTask, name) =
        { task with AzureAppServiceManageRawTask = { task.AzureAppServiceManageRawTask with AppName = Some name } }

    [<CustomOperation "slot">]
    member _.AddSlot(task: AzureAppServiceManageStartRawTask, slot) =
        { task with Slot = Parameter.Set slot }

    interface ITaskBuilder<AzureAppServiceManageStartRawTask> with
...
let azureAppServiceManageStart = AzureAppServiceManageStartTaskBuilder()
